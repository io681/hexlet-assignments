...
public class ProductSpecification {
    public Specification<Product> build(ProductParamsDTO params) {
        return withCategoryId(params.getCategoryId()).and(withPriceGt(params.getPriceGt()))
                ...
                .and(withContainName(params.getTitleCont()));
    }
    ...
    private Specification<Product> withContainName(String name) {
        return (root, query, cb) -> name == null ? cb.conjunction() : cb.like(root.get("title"), "%" + name.toLowerCase() + "%");
